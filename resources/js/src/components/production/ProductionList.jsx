...
import { DASHBOARD_PATH, PRODUCTION_ADD_PATH, PRODUCTION_VIEW_PATH} from "../../router";
import { EditIcon, DeleteIcon, ViewIcon } from "@chakra-ui/icons";
import Swal from "sweetalert2";
import { Link as ReactRouterLink } from "react-router-dom";
import { PRODUCTION_EDIT_PATH } from './../../router';
...
export default function ProductionList() {
    ...
    const fetchRecipes = async () => {
        try {
            ...
            const res = await api.get("/superadmin/productions", {
                params: {
                    page: pageIndex + 1,
                    per_page: pageSize,
                    search: globalFilter || "",
    ...
    useEffect(() => {
        ...
        document.title = `${app_name} | Production Management`;
        fetchRecipes();
    ...
    const deleteProduction = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                await api.delete(`superadmin/productions/${id}`);
                toast({
                ...
                <>
                    <Box display="flex" gap={2}>
                        <ChakraLink
                            ...
                            borderRadius="md"
                            onClick={() =>
                                navigate(PRODUCTION_VIEW_PATH(row.original.id))
                            }
                        >
                            <ViewIcon />
                        </ChakraLink>
