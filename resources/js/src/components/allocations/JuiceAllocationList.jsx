...
export default function JuiceAllocationList() {
    ...
    const fetchIngredients = async () => {
        try {
            ...
            const res = await api.get("/superadmin/driver/juice/allocations", {
                params: {
                    page: pageIndex + 1,
                    per_page: pageSize,
                    search: globalFilter || "",
    ...
    const deleteAllocation = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                await api.delete(`superadmin/driver/juice/allocations/${id}`);
                toast({
                ...
                <>
                    <Box display="flex" gap={2}>
                        <ChakraLink
                            ...
                            borderRadius="md"
                            onClick={() =>
                                navigate(JUICE_ALLOCATION_EDIT_PATH(row.original.id))
                            }
                        >
                            <EditIcon />
                        </ChakraLink>
                        ...
                        <ChakraLink
                            ...
                            borderRadius="md"
                            cursor="pointer"
                            onClick={() => deleteAllocation(row.original.id)}
                        >
                ...
                <Card>
                    <CardBody>
                        <TanStackTable
                            ...
                            pageCount={pageCount}
                            isLoading={isLoading}
                            addURL={JUICE_ALLOCATION_ADD_PATH}
                        />
                    </CardBody>
                </Card>
            </SimpleGrid>
