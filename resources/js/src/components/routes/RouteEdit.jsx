...
const RouteEdit = () => {
  ...
  const getDriverRoutes = async () => {
    try {
      const res = await api.get(`superadmin/driver/routes/${id}`);
      const driver_route = res.data.data;

        setForm({
            user_id: driver_route.user_id ?? "",
            day: driver_route.day ?? ""
        });

        const decodedLocations = JSON.parse(driver_route.locations || "[]");

        setItems(
            decodedLocations.map((line) => ({
  ...
  useEffect(() => {
    ...
    document.title = `${app_name} | Edit Driver Route`;
    getLocations();
    getDrivers();
    getDriverRoutes();
  }, []);
  ...
  const onSubmit = async () => {
    ...
    try {
      const res = await api.put(`superadmin/driver/routes/${id}`, payload);
      toast({
        position: "bottom-right",
        title: res.data.message || "Route updated successfully",
        status: "success",
        duration: 3000,
        isClosable: true,
      });
